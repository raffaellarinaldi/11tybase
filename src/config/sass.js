...
module.exports = (eleventyConfig, dirs, options = {}) => {
  const baseCssPath = options.baseCssPath || '/assets/css'
  ...
  eleventyConfig.on('afterBuild', () => {
    if (isProduction) {
      const outputPath = path.join(dirs.output, baseCssPath)

      try {
        const cssFiles = fs.readdirSync(outputPath).filter(file => 
          file.endsWith('.css') && !file.endsWith('.min.css') && file.startsWith('styles')
        )
