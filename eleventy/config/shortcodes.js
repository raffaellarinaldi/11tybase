...
module.exports = (eleventyConfig) => {
  ...
  eleventyConfig.addShortcode('createdBy', function (lang = 'en', className = '', extraLinks = []) {
    const effectiveLang = lang || this.ctx?.environments?.lang || this.ctx?.lang || 'en'
    ...
    const mainLink = `<a href="https://raffaellarinaldi.com" ${commonAttrs}>RRWebAgency</a>`
  ...
  eleventyConfig.addShortcode('consentButtons', (lang = 'en', buttons = []) => {
    ...
    return buttons.map(([action, className = '', extra = '']) => {
      const dataCc = {
        accept: 'accept-all',
      ...
      return `<button class="${className}" data-cc="${dataCc}" ${extra}>${label}</button>`
    }).join('\n')
  })
}
