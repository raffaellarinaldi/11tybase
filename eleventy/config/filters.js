...
module.exports = (eleventyConfig) => {
  ...
  eleventyConfig.addFilter('getItemsByPosition', (input, ...positions) => {
    if (!input) return []
    const arr = Array.isArray(input) ? input : Object.values(input)
    return positions.map(pos => arr[pos - 1]).filter(Boolean)
  })
  ...
  eleventyConfig.addFilter('excludePostsFromArchive', (posts) => {
    if (!Array.isArray(posts)) return []
    return posts.filter(post => !post.data.excludeFromArchive)
  })
  ...
  eleventyConfig.addFilter('filterItemsByKey', (input, key, value, negate = false) => {
    if (!input || typeof input !== 'object') return []
    const arr = Array.isArray(input) ? input : Object.values(input)
  
    return arr.filter(item => {
      const hasKey = item && key in item
      const matchesValue = value === undefined || item[key] === value
  
      return negate ? !hasKey : hasKey && matchesValue
    })
  })
